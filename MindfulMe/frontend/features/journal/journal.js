class Journal {
    ...
    async saveEntry() {
        ...
        try {
            if (this.recordedBlob) {
                // Use XHR for upload progress tracking — same-origin path
                await this.uploadWithProgress('/api/journal', formData);
            } else {
    ...
    createEntryElement(entry) {
        ...
        if (entry.media_file_path) {
            // Use root-relative path — served from same origin
            const mediaUrl = `/${entry.media_file_path}`;
        ...
        try {
            ...
            if (videoLogs.length) {
                ...
                videoLogs.forEach(log => {
                    ...
                    if (!stats[emotion]) stats[emotion] = { count: 0, totalConf: 0 };
                ...
                html += `
                    <div class="emotion-card emotion-card--wide">
                        ...
                        <div class="emotion-card__stats-row">
                            <div class="stat-box"><span class="stat-box__value">${totalSamples}</span><span class="stat-box__label">Samples</span></div>
                            <div class="stat-box"><span class="stat-box__value">${duration}s</span><span class="stat-box__label">Duration</span></div>
                            <div class="stat-box"><span class="stat-box__value" style="color:${domClr.fg}">${dominant.emotion}</span><span class="stat-box__label">Dominant</span></div>
                            <div class="stat-box"><span class="stat-box__value">${summary.length}</span><span class="stat-box__label">Emotions</span></div>
                        </div>
